const app = {
    ...
    async getSoldOutProduct() {
        const data = await app.fetchData();
        const soldOutProduct = data.items.filter(product => product.variants.every(variant => variant.available === 0))
        if (soldOutProduct.length === 0) return 'Khong co san pham nao het hang'
        return soldOutProduct.map(product => {
            return `
                <h2>${product.title}</h2>
            `
        }).join('')
